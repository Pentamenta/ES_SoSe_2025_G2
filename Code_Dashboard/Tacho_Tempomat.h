...
void writeTacho(uint16_t speed){
  ...
  for (int angle = -180; angle <= 0; angle += 15) { //geht einen Halbkreis in 15° Grad Schritten ab
    ...
    mdisplay.drawLine(x1, y1, x2, y2);               //zeichnet eine Linie von (x1|y1) zu (x2|y2)   
  }
  ...
  for (int angle = -180; angle <= 0; angle += 30) { //geht die Winkel in 30° Schritten durch, sodass bei jedem zweiten Strich ein Wert stehen wird
    ...
    mdisplay.setFont(u8g2_font_5x7_tr);              //Schriftgröße + Font bestimmen
    mdisplay.setCursor(x - 6, y);                    //setzt Anfangsposition für Wert, der per print() auf Display angezeigt wird
    mdisplay.print(value);                           //Wert auf Display an berechneter Position anzeigen
  }
  ...
  mdisplay.drawLine(centerX, centerY, needleX, needleY); //Linie vom Mittelpunkt zum Endpunkt der Nadel zeichnen -> Tachonadel entsteht
}
#endif